...
(function(){
  ...
  function toJSON(form){
    ...
    // optional: normalize a few known fields if present
  ...
  function openPrintPopup(){
    // open synchronously to preserve user gesture
  ...
  function currentModeFrom(actionUrl){
    try{
      var u = new URL(actionUrl, location.href);
      return (u.searchParams.get('mode')||'').toLowerCase()==='print' ? 'print' : 'save';
    }catch(e){ return 'save'; }
  }
  ...
  function attach(){
    ...
    form.addEventListener('submit', async function(e){
      ...
      var submitter = e.submitter || lastSubmitter || form.querySelector('[type="submit"]');
      var actionUrl = (submitter && submitter.formAction) ? submitter.formAction : form.getAttribute('action') || ABS_ENDPOINT;
      var mode = currentModeFrom(actionUrl);
      ...
      try{
        ...
        if (mode === 'print'){
          try{
            var f = (w && w.document.getElementById('pv')) || null;
            if (f) f.src = url; else if (w) w.location.href = url; else window.open(url, '_blank');
